...
class HomeoLabelApp(QtWidgets.QWidget):
    ...
    def get_records_file(self):
        """Ensure records.xlsx exists in user's AppData folder and return its path."""
        appdata_folder = os.path.join(os.getenv('APPDATA'), 'HomeoLabelApp')
        os.makedirs(appdata_folder, exist_ok=True)
        records_file = os.path.join(appdata_folder, 'records.xlsx')

        if not os.path.exists(records_file):
            # Copy default records.xlsx if exists
            default_records = os.path.join(os.getcwd(), 'records.xlsx')
            if os.path.exists(default_records):
                shutil.copy(default_records, records_file)
            else:
                pd.DataFrame([]).to_excel(records_file, index=False, engine='openpyxl')
        return records_file
    ...
    def init_ui(self):
        ...
        self.preview_line2 = QtWidgets.QLabel("")  # Second line for medicine
    ...
    def update_selected_medicine(self):
        ...
        if med_name.strip() == "":
            QMessageBox.warning(self, "Empty Medicine", "Medicine name cannot be empty!")
            return
    ...
    def update_preview(self):
        med_name = self.medicine_search.text().upper()
        if med_name.strip() == "":
            self.preview_line1.setText("")
            self.preview_line2.setText("")
            return
        ...
        # Split medicine name across 2 lines if too long
        split_idx = len(med_name)//2
        # Try to split at space near middle
        space_idx = med_name.rfind(' ', 0, split_idx)
        if space_idx != -1:
            split_idx = space_idx
        self.preview_line1.setText(med_name[:split_idx])
        self.preview_line2.setText(med_name[split_idx:] + f" {potency}")
    ...
    def generate_pdf(self, pdf_file):
        med_name = self.medicine_search.text().upper()
        if med_name.strip() == "":
            QMessageBox.warning(self, "Empty Medicine", "Cannot print empty medicine name!")
            return
        ...
        # Save record to user AppData Excel
        record = {
            "Medicine": med_name,
            "Potency": potency,
            "Dose": dose,
        ...
        records_path = self.get_records_file()
        if os.path.exists(records_path):
            df_records = pd.read_excel(records_path, engine="openpyxl")
        ...
        # Two lines for medicine name
        split_idx = len(med_name)//2
        space_idx = med_name.rfind(' ', 0, split_idx)
        if space_idx != -1:
            split_idx = space_idx
        c.drawCentredString((width_mm/2)*mm, y, med_name[:split_idx])
        y -= 5*mm
        c.drawCentredString((width_mm/2)*mm, y, med_name[split_idx:] + f" {potency}")
