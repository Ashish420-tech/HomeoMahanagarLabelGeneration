...
import subprocess  # For direct printer command
...
class HomeoLabelApp(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.top_offset_slider.setRange(0, 100)
        ...
        self.direct_print_btn = QtWidgets.QPushButton("Print LP46 Neo")
    ...
    def update_label_settings_from_slider(self):
        self.top_offset = self.top_offset_slider.value() / 100.0
        self.update_preview()
    ...
    def print_direct(self):
        try:
            pdf_file = os.path.join(self.records_folder, "label.pdf")
            self.generate_pdf(pdf_file)
            # Example for Windows using lp46 driver via default PDF reader print command
            subprocess.run(['AcroRd32.exe', '/t', pdf_file, 'LP46 Neo'], check=True)
    ...
    def generate_pdf(self, pdf_file):
        ...
        # Generate PDF 48x28 mm (~1.89x1.10 inch)
        c = canvas.Canvas(pdf_file, pagesize=(1.89*inch, 1.10*inch))
        c.setLineWidth(1)
        c.rect(0.05*inch, 0.05*inch, 1.79*inch, 1.0*inch)
