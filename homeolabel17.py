...
class HomeoLabelApp(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.preview_frame.setFixedSize(400, 280)
        ...
        # Top Offset
        self.top_offset_spin = QtWidgets.QDoubleSpinBox()
        self.top_offset_spin.setRange(0.0, 1.0)
        self.top_offset_spin.setSingleStep(0.01)
        self.top_offset_spin.setValue(self.top_offset)
        self.top_offset_spin.setSuffix(" in")
        self.top_offset_spin.valueChanged.connect(self.update_label_settings)
        settings_layout.addWidget(QtWidgets.QLabel("Top Offset:"))
        settings_layout.addWidget(self.top_offset_spin)
        ...
        self.print_direct_btn = QtWidgets.QPushButton("Direct Print LP46 Neo")
        self.print_direct_btn.clicked.connect(self.print_direct)
        preview_btn_layout.addWidget(self.print_direct_btn)
    ...
    def update_label_settings(self):
        self.top_offset = self.top_offset_spin.value()
    ...
    def update_preview(self):
        ...
        time_val_formatted = " ".join(time_val.split())

        self.preview_line1.setText(f"{med_name} {potency}")
        self.preview_line2.setText(f"{quantity}")
        self.preview_line3.setText(f"{dose}   {time_val_formatted}")
    ...
    def print_label(self):
        self.generate_pdf(os.path.join(self.records_folder, "label.pdf"))
        # Open PDF
        ...
        self.status.setText("Label generated and PDF opened.")

    # ------------------ Direct Print LP46 Neo ------------------
    def print_direct(self):
        try:
            pdf_file = os.path.join(self.records_folder, "label.pdf")
            self.generate_pdf(pdf_file)
            printer_name = "LP46 Neo"
            ...
            self.status.setText("Label sent to LP46 Neo printer.")
        except Exception as e:
            logging.error(f"Direct print failed: {e}")
            QtWidgets.QMessageBox.critical(self, "Error", f"Direct print failed: {e}")
    ...
    def generate_pdf(self, pdf_file):
        ...
        for field, value in [("potency", potency), ("dose", dose),
                             ("time", time_val), ("shop", shop), ("branch_phone", branch_phone)]:
            if value:
        ...
        # Generate PDF
        c = canvas.Canvas(pdf_file, pagesize=(1.89*inch, 1.10*inch))  # 48x28 mm approx
        c.setLineWidth(1)
        c.rect(0.05*inch, 0.05*inch, 1.79*inch, 1.0*inch - self.top_offset*inch)

        y = 1.0*inch - self.top_offset*inch
