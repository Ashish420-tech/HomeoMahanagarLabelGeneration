...
class HomeoLabelApp(QtWidgets.QWidget):
    ...
    def update_preview(self):
        ...
        self.preview_line1.setText(med_name[:len(med_name)//2])
        self.preview_line2.setText(med_name[len(med_name)//2:] + f" {potency}")
    ...
    def generate_pdf(self, pdf_file):
        ...
        # Two lines for medicine name
        c.drawCentredString((width_mm/2)*mm, y, med_name[:len(med_name)//2])
        y -= 5*mm
        c.drawCentredString((width_mm/2)*mm, y, med_name[len(med_name)//2:] + f" {potency}")
