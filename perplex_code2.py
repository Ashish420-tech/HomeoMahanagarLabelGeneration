...
class HomeBox(BoxLayout):
    status_text = StringProperty('Ready. Load remedies.xlsx (with latin_col and common_col columns).')
    ...
    def _load_df(self, path):
        ...
        if 'latin_col' not in cols or 'common_col' not in cols:
            raise ValueError("Excel file must have 'latin_col' and 'common_col' columns")
        # Correct for cases where column names aren't stripped
